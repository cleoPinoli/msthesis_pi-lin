...
now formalise in Agda. First of all we need to define the notion of
\emph{thread}, that is any process other than a cut. As it turns out, it is
convenient to provide a more fine-grained classification of threads,
...
isomorphic to the relevant fragments of the \AgdaDatatype{Proc} datatype. The
only aspect that is worth pointing out here is that in \AgdaDatatype{Input},
...
cut or on a free channel. To clarify this aspect, consider the
\AgdaInductiveConstructor{wait} constructor in \AgdaDatatype{Input} and in
...
We define observability, reducibility and aliveness in the expected way:
