...
forms described in \Cref{sec:types}. We start by defining an indexed data type
$\AgdaDatatype{PreType}~n$ to represents (\LCP) types in the scope of $n$
...
either globally when invoking Agda or within a pragma in the module's source
code.}
...
always substitute one variable at a time, but it is technically easier to
substitute \emph{all} variables of a pre-type, possibly with themselves. The
definition of \AgdaFunction{subst} relies on some auxiliary functions for
\emph{renaming} type variables and \emph{lifting} substitutions across
quantifiers. These functions are straightforward adaptations of those described
by \citet{KokkeSiekWadler20}.
...
$\AgdaFunction{subst}~\sigma~(\AgdaFunction{dual}~(\AgdaInductiveConstructor{rav}~x))
  ...
  which is definitionally the same as $\sigma~x \equiv
