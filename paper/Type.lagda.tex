...
is worth defining an implicit rewriting rule so that Agda can autonomously apply
it whenever possible. This is achieved by means of the following pragma
directive.
...
definitions are straightforward adaptations of those describe by
\cite{KokkeWadler}.
...
At this stage we are finally ready to define the type of closed pre-types, those
having no free type variables. We call \AgdaDatatype{Type} closed pre-types.
Hereafter, we will seldom use pre-types again.
