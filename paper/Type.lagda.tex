...
forms described in \Cref{sec:types}. We start by defining \emph{pre-types} as an
indexed data type $\AgdaDatatype{PreType}~n$ where $n$ is the number of bound
...
This property is so important in the rest of the formalisation that it is worth
...
\texttt{{-}{-}rewriting} is enabled, either globally when invoking Agda or
within a pragma in the module's source code.}
