...
\AgdaFunction{+-assoc-l} in \Cref{sec:context-agda}.\Luca{Controllare
left/right.} The nested \AgdaKeyword{let}-\AgdaKeyword{in} allows us to pattern
match the result of the application $\AgdaFunction{+-assoc-l}~p~q$ and to
extract the new proofs $p'$ and $q'$ for the rearranged splittings.
...
respectively $\InTag_1$ and $\InTag_2$.\Luca{Controllare nomi dei tag con
sintassi del calcolo. Chiamarli inj come in Agda così c'è consistenza?}
%
...
>}~q~\AgdaInductiveConstructor{⟩})~\AgdaInductiveConstructor{inj₁}~P)$, which is
