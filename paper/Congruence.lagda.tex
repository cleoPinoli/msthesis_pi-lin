...
proof $\AgdaInductiveConstructor{>}~q$ used within
\AgdaInductiveConstructor{wait}. To understand the meaning of this proof, we
must recall three key elements:
\begin{enumerate}
  \item
...
channel $y : B \Plus C$ to select $\InTag_1$.
%
The continuation process $P$ uses a fresh continuation channel $z : B$.
...
the type $B$ of $z$ comes \emph{before} the type $A$ of $x$ since $z$ is
introduced later than $x$.
%
After structural pre-congruence is applied, however, the type of the
continuation channel $z$ ends up behind that of the restricted channel $x$,
because now $z$ and $x$ are introduced in the opposite order. Therefore, we need
to rename the channels in $P$ so that it is well typed in the context
...
Recall from \Cref{tab:semantics} that we allow using this rule on a process of
the form $\Cut[A]\x{\Fork\y\u\v{P}{Q}}{R}$ when $x \in \fn{P}$. We capture the
condition $x\in\fn{P}$ by means of the splitting
$\AgdaInductiveConstructor{<}~r$, implying that the type $A$ of $x$ ends up in
the typing context for $P$ and not in the one for $Q$.
%
The symmetric rule \SForkR is modeled by another constructor
\AgdaInductiveConstructor{s-fork-r}, which is similar to
...
mind that, using an intrinsically-typed representation of processes, we have
