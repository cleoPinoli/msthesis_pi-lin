...
Just like structural pre-congruence, reduction is formalised as a binary
relation between processes that are well typed in the same typing context. Thus,
the definition of reduction embeds subject reduction (\Cref{thm:red}).
...
Let us comment on a few representative cases.
...
The splitting $p$ indicates that $y$ (of type $\dual{A}$) occurs in the left
...
with the syntax of the link being reduced, which is oriented so that the
restricted channel $x$ is on the left. 
%
...
$\Context$. The exact location of $y$ in $\ContextC$ is encoded in the splitting
...
While the process constructor \AgdaInductiveConstructor{close} implicitly refers
to the only free channel occurring in a process of the form $\Close\x$, the
constructor \AgdaInductiveConstructor{wait} uses a splitting proof of the form
$\AgdaInductiveConstructor{<}~q_0$ to make sure that the referenced channel is
...
partition is empty, these splittings must be equal and made of a sequence of
\AgdaInductiveConstructor{>} applications followed by
\AgdaInductiveConstructor{•}. In general, Agda will not be able to ``see'' that
they are definitionally equal, hence it is easier to quantify over them separately so
that we do not have to prove their equality whenever we wish to apply this
reduction.
...
pattern, except for the implementation of \RWeaken and \RContract which require
auxiliary functions to respectively weaken and contract the typing context of
...
In order for these two processes to be composable in a cut, it must be the case
