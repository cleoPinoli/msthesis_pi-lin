...
\Cref{tab:comparison} reports a summary of the aforementioned formalisations
(sorted by publication date) and also our own. The first five columns identify
the calculus being formalised specifying its reference paper, the used proof
assistant and an acronym that gives an idea of the overall flavour of the
calculus.
%
The acronyms \CP~\cite{Wadler14,GayVasconcelos25} and
\GV~\cite{GayVasconcelos10,Wadler14} are well-known in the literature on session
types. The acronym \SessionPI refers to (variants of) the session-based
$\pi$-calculus presented by \citet{HondaVasconcelosKubo98} while \LinearPI
refers to (variants of) the linear
...
the same acronym may refer to different versions of the same calculus.
%
We also indicate whether the calculus features \emph{cuts} (that is, the
combination of restriction and parallel composition in correspondence with the
...
result of the operation (if any) \emph{along with the same channel $s$}. In this
way, the type of $s$ can be conveniently ``updated'' to take into account the
effect of the operation. As observed by \citet{Padovani17}, this semantics of
the communication primitives is virtually indistinguishable from one making use
of explicit continuation channels.
...
contexts, the representation of channels and whether the formalisation is
intrinsically or extrinsically typed.
...
Looking on the available data, we can make the following observations.
%
