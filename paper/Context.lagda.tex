...
contexts are represented using lists of types, where the (polymorphic)
...
$\ContextC$, $\ContextD$ and $\ContextE$ to range over typing contexts as in
\Cref{sec:typing-rules}, even though in the Agda formalisation they are lists
instead of finite maps.
...
those contexts $\ContextC$ that can be split into $\ContextD$ and $\ContextE$
such that $P$ holds for $\ContextD$ and $Q$ holds for $\ContextE$.
%
