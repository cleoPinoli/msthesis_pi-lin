...
$\ContextC$, $\ContextD$ and $\ContextE$ to range over typing contexts as we
have done in \Cref{sec:typing-rules}, even though in the Agda formalisation they
are lists instead of finite maps.
...
\AgdaFunction{≫} will be provided in \Cref{sec:examples}.
...
point that it contributes substantially to the complexity of the formalisation.
For this reason, we invest some effort in defining a few abstractions that allow
us to hide context splittings and enhance code readability.
%
...
$P~\ContextD$ and $Q~\ContextE$.
%
The use of the metavariables $P$ and $Q$ for denoting predicates over contexts
is not without reason. As we will see shortly, in our formalisation processes
are indeed predicates over typing contexts, albeit not the only ones.
