...
\begin{code}
EchoServerT : Type
EchoServerT = `! (`∀ (rav (# 0) ⅋ (var (# 0) ⊗ 𝟙)))
\end{code}

The notation $\AgdaFunction{\#}~n$ is defined in Agda's standard library and
creates an element of \AgdaDatatype{Fin} from the natural number $n$. In
\AgdaFunction{EchoServer}, we use it to represent the de Bruijn index of the
type variable $X$.
%
We now encode the server
\begin{code}
Server : Proc [ EchoServerT ]
...
\begin{code}
Client : Proc (dual EchoServerT ∷ 𝔹 ∷ [])
...
To test these definitions, we define a main process that connects server and
client
\begin{code}
Main : Proc [ 𝔹 ]
