...
According to our nameless representation of channels, the \emph{position} of a
type in a typing context $\Context$ determines the location of its binder in the
structure of a process. When the binding structure of a process changes, \eg
...
\Cref{tab:semantics}, $\Context$ must be suitably rearranged to agree with the
updated binding structure. Such rearrangement is in fact a \emph{permutation} of
...
It is easy to see that \AgdaDatatype{\_↭\_} is an equivalence relation. In the
following we also use another property of permutations related to context
splitting and list concatenation \AgdaFunction{\_++\_}: if
