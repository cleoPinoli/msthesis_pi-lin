...
The definition of \AgdaDatatype{\_↭\_} data type is nearly the same found in
...
more importantly, the data type is \emph{monomorphic} (it does not need to
relate lists of arbitrary type) and the quantified arguments $A$ and $B$ of
\AgdaInductiveConstructor{swap} and \AgdaInductiveConstructor{prep} are
implicit, which streamlines their usage in the rest of the code.
...
enjoys a number of expected properties, such as symmetry. Among them, in the
following we will make use of a specific one that connects context splitting and
permutations: if $\Splitting\ContextC\ContextD\ContextE$, then $\ContextC$ is a
permutation of the concatenation of $\ContextD$ and $\ContextE$.\Luca{Ci sono
molte altre proprietà non descritte, controllare quelle che servono davvero.}
