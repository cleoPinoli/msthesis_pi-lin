...
#rot : ∀{n} {A B C : Type n} {Γ : Context n} → (A ∷ B ∷ C ∷ Γ) # (C ∷ A ∷ B ∷ Γ)
#rot = #tran (#next #here) #here

#cons : ∀{n} {A : Type n} {Γ Δ : Context n} → Γ ≃ A , Δ → (A ∷ Δ) # Γ
#cons (split-l p) with +-empty-l p
... | refl = #refl
#cons (split-r p) = #tran #here (#next (#cons p))

#split : ∀{n} {Γ Γ₁ Γ₂ Δ : Context n} → Γ # Δ → Γ ≃ Γ₁ + Γ₂ → ∃[ Δ₁ ] ∃[ Δ₂ ] (Δ ≃ Δ₁ + Δ₂ × Γ₁ # Δ₁ × Γ₂ # Δ₂)
...
#one+ : ∀{n} {A : Type n} {Γ Γ′ Δ : Context n} → Γ # Δ → Γ ≃ A , Γ′ → ∃[ Δ′ ] (Δ ≃ A , Δ′ × Γ′ # Δ′)
#one+ π p with #split π p
... | _ , _ , q , π₁ , π₂ rewrite #singleton-inv π₁ = _ , q , π₂

#shift : ∀{n} {A : Type n} {Γ Δ : Context n} → (Γ ++ A ∷ Δ) # (A ∷ Γ ++ Δ)
#shift {_} {_} {[]} = #refl
#shift {_} {_} {B ∷ Γ} = #tran (#next #shift) #here

+++# : ∀{n} {Γ Γ₁ Γ₂ : Context n} → Γ ≃ Γ₁ + Γ₂ → (Γ₁ ++ Γ₂) # Γ
+++# split-e = #refl
+++# (split-l p) = #next (+++# p)
