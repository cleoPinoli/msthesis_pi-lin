...
channels in systems of communicating processes, and the need for coinductive
...
on peer-to-peer communications. Every session-typed system revolves around three
key ideas: (1) session endpoints \REVISION{can only be used \emph{sequentially}
and cannot be} discarded or duplicated without compromising some safety and
liveness properties of a program; (2) the type of a session endpoint is
\emph{updated} after each use to reflect the state of the protocol it describes;
...
\emph{continuations} -- on which the rest of the communication takes place.
%
\REVISION{Roughly speaking, \LCC stands in between \CP~\cite{Wadler14} --
Wadler's calculus of sessions based on classical linear logic -- and the
interpretation of linear logic proofs as $\pi$-calculus processes studied by
Abramsky~\citep{Abramsky93} and by Bellin and Scott~\citep{BellinScott94}. While
\CP features native sessions, \LCC and the interpretation by Abramsky, Bellin
and Scott~\cite{Abramsky93,BellinScott94} make use of explicit continuations.
While both \CP and \LCC provide native process forms corresponding to the
additive connectives of the logic, Abramsky, Bellin and
Scott~\cite{Abramsky93,BellinScott94} encode them using affine --
``use-at-most-once'' -- channels.}
...
the same (sequential) process to own both endpoints of a session, which is
undesirable since it does not correspond to a useful pattern of interaction
(every meaningful session requires its endpoints to be used by parallel
processes) and is a potential source of deadlocks.
%
...
need to \emph{update} the type of channels in typing contexts. \REVISION{Once a
channel has been used it is effectively consumed, and therefore its type is
\emph{removed} from the typing context. The continuation channel is fresh by
definition, and therefore its type is \emph{added at the beginning} of the
typing context. As it turns out, removing and adding types is easier than
updating them.}

The formalisation of \LCC that we obtain is at the same time complete (in terms
of features supported by the calculus) and streamlined (in terms of code size)
compared to the known formalisations of session/linear
...
assume that the reader is somewhat familiar with Agda, but we recall the lesser
known definitions from Agda's standard library.
%
\REVISION{\Cref{sec:equivalence} provides a more detailed description of the
correspondence between \CP and \LCC.}
%
\Cref{sec:related} discusses related work more in detail and includes a
...
The formalisation has been checked with Agda 2.8.0 and the code is available in
a public repository on GitHub~\cite{PadovaniRaffaelli25}.
