...
challenge}, concerns the formalization of a minimal \emph{session calculus}.
%
...
techniques for dealing with typing contexts without splitting them, including
leftover typing~\cite{ZalakainDardha21}, the use of linearity
predicates~\cite{SanoKavanaghPientka23} and contexts with
tags~\cite{ZackonEtAl25}.
...
challenge description differ for two key aspects.
%
...
higher-order, polymorphic sessions and both linear and shared channels. Also, as
suggested by its name, \LCP is closely related to \CP~\citep{Wadler14} and its
type system is based on classical linear logic.
%
The second difference is that the calculus in the challenge is equipped with
\emph{native} sessions, whereas \LCP is a \emph{linear} calculus where each
channel is meant to be used for a \emph{single} communication. Structured
sessions involving the exchange of several messages can be modeled thanks to the
use of \emph{explicit continuation channels}. This encoding of (binary) sessions
using linear channels is well known in the
literature~\citep{KobayashiPierceTurner99,Kobayashi02b,DardhaGiachinoSangiorgi17}.
%
Overall, \LCP has the same expressive power of \CP, but is a lower-level
calculus in which sessions can be encoded instead of being featured natively.

The formalization we present is the most complete (in terms of features
supported by the calculus) and, at the same time, the simplest one (in terms of
code size). It is also one of only two formalizations that prove the deadlock
freedom property, and it does so with an order of magnitude less code than the
