...
key ideas: (1) session endpoints can only be used \emph{sequentially} and cannot
be discarded or duplicated without compromising some safety and liveness
properties of a program; (2) the type of a session endpoint is \emph{updated}
after each use to reflect the state of the protocol it describes; (3) peer
...
%
Roughly speaking, \LCC stands in between \CP~\cite{Wadler14} -- Wadler's
calculus of sessions based on classical linear logic -- and the interpretation
of linear logic proofs as $\pi$-calculus processes studied by
Abramsky~\citep{Abramsky93} and by Bellin and Scott~\citep{BellinScott94}. While
\CP features native sessions, \LCC and the interpretations by Abramsky, Bellin
...
has been used it is effectively consumed, and therefore its type is
...
%
\Cref{sec:equivalence} provides a more detailed description of the
