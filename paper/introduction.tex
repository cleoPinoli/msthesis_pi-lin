...
alternatives techniques including leftover typing~\cite{ZalakainDardha21}, the
...
\emph{instead of describing new techniques that are effective for the
formalization of the calculus in the challenge, we describe a new calculus that
is easy to formalize with known techniques}.\Luca{dobbiamo dire perché lo
spirito della challenge è rispettato, e dunque giustificare il titolo
dell'articolo. Dire che il nostro non è un calcolo ad hoc, ma ha fondamenti
logici importanti, è completo, ed è strettamente in relazione con altri modelli
analoghi (CP e linear pi)}
%
More specifically, we present a calculus called \LCP whose type system coincides
with the proof system of \emph{classical linear logic} and that features
\emph{linear channels} instead of sessions. Linear channels are meant to be used
for a \emph{single} communication. Structured sessions involving the exchange of
several messages can be modeled in \LCP using \emph{explicit continuations}
according to the well-known encoding of (binary) sessions into the linear
$\pi$-calculus~\citep{KobayashiPierceTurner99,Kobayashi02b,DardhaGiachinoSangiorgi17}.
%
Overall, \LCP is a low-level calculus with a strong connection with linear logic
and in which sessions are encodable instead of being featured natively.
%
...
In the end, the formalization of \LCP is both the most complete (in terms of
