...
\>[0]\AgdaFunction{Client}\AgdaSpace{}%
\AgdaSymbol{=}%
\>[287I]\AgdaFunction{curry∗}\AgdaSpace{}%
...
\begin{remark}
    Writing processes in Agda would be easier if we could avoid using
    \AgdaFunction{curry∗}, namely if the constructors of the data type
    \AgdaDatatype{Proc} were naturally curried.
    %
    This is possible, but only if we are willing to sacrifice some clarity. To
    illustrate the problem with an example, below is the fully curried
    constructor \AgdaInductiveConstructor{fork} of a hypothetical data type
    \AgdaDatatype{Proc'}, obtained by expanding the definition of separating
    conjunction:
\begin{code}[hide]%
...
\end{code}
    ...
    This version of \AgdaInductiveConstructor{fork} is way less readable than
    the one we gave in \Cref{sec:process-agda} because of the (now visible)
    context splittings. We can recover some clarity and still obtain a curried
    version of \AgdaInductiveConstructor{fork} using the magic wand instead of
    Agda's native arrow:
\begin{code}[hide]%
\>[0]\AgdaKeyword{data}\AgdaSpace{}%
\AgdaDatatype{Proc''}\AgdaSpace{}%
...
\>[2]\AgdaInductiveConstructor{fork}\AgdaSpace{}%
\AgdaSymbol{:}%
\>[391I]\AgdaSymbol{∀\{}\AgdaBound{A}\AgdaSpace{}%
\AgdaBound{B}\AgdaSymbol{\}}\AgdaSpace{}%
\AgdaSymbol{→}\<%
\\
\>[.][@{}l@{}]\<[391I]%
...
\end{code}
    ...
    However, now in each constructor we have to make a distinction between the
    first arrow \AgdaFunction{⇒} and the following magic wands \AgdaFunction{─∗}
    to account for the appropriate amount of context splittings. We found this
    formulation of the constructors harder to explain and motivate in
    \Cref{sec:process-agda}.
    %
    In the end, none of the alternative definitions of the data type
    \AgdaDatatype{Proc} was fully satisfactory and we preferred the cleanest and
    most readable version at the expense of additional clutter in this section.
    %
    \eoe
\end{remark}
