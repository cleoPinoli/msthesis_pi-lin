...
| t_throw : forall G k k' P D T T',
    oft G P (add k T' D) ->
    def (add k (ch_output T T') (add k' T D)) -> 
...
| t_nu_nm : forall (L : seq SC.atom) G s P D, 
    (forall x, x \notin L ->
...
Proof.
  ...
  { 
    ...
    intros.
    rewrite/subst_proc_exp=>//. rewrite-/subst_proc_exp. 
...
Proof.
  ...
  +  apply: t_bang.
    move: CompD cD1; rewrite /completed/binds. 
    move=> A B .
    split=>// a.
    rewrite dom_union Def/= =>/orP-[|]H.
    - 
